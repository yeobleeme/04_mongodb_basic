...
// 5. update
// 1) updateOne()
db.posts.find({ title: "제목 1" });
db.posts.updateOne({ title: "제목 1" }, {$set: {likes: 2}});
db.posts.find({ title: "제목 1" });

// 2) updateOne()후 not found일경우
db.posts.updateOne({ title: "제목 5" }, 
	{
		$set: 
			{
				title: "[수정]제목 5",
...
// 6. delete
// 1) deleteOne()
db.posts.deleteOne({title: "[수정]제목 5"});
db.posts.find();
...
//	$pop      : 처음이 마지막 요소를 꺼내온 후에 삭제
//	$pull     : 조건에 일치된 모든 요소를 삭제
//	$push     : 요소를 추가
...
// select categroy, sum(likes) from posts group by category;
db.posts.aggregate(
	{$match: {likes: {$gt: 1}}},
...
// 2) $limit : 573a1390f29313caabcd4135
...
// 4) $sort
// select name, accommodates from listingsAndReviews order by accommodates desc;
...
// 5) $match
// select name, bedrooms, price from listingsAndReviews
//  where property_type = 'House' limit 2;
db.listingsAndReviews.aggregate(
...
// 8) $lookup(from, localField, foreignFildes, as)
// from : 동일 DB안에 lookup할 collection
// localField: collection에서 unique한 identifier사용
// foreighFields: 참조collection primay필드 (PK)
// as : 새로운 필드명
db.comments.find();
...
db.listingsAndReviews.aggregate(
	{$group: {
		_id: "$proper_type",
		properties: {
			$push: {name: "$name", accomodates: "$accomodates", price: "price"}
		}
	}},
	{$out: "properties_by_type"}
);

// C. embedded/nested documents
...
db.embdded.find();
db.embdded.find({size: {w:21, h:14, uom: "cm"}}); // 순서가 틀림
db.embdded.find({size: {h:14, w:21, uom: "cm"}});
db.embdded.find({size: {h:14, w:21, uom: 'cm'}});
...
db.enableFreeMonitoring(); // mongoCompas의 mongoshell에서 실행후 주소카피해서 크롬주소창에서 실행
